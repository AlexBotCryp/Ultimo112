...
assert API_KEY, "BINANCE_API_KEY no está definido"
assert API_SECRET, "BINANCE_API_SECRET no está definido"
assert TELEGRAM_TOKEN, "TELEGRAM_BOT_TOKEN no está definido"
assert TELEGRAM_CHAT_ID, "TELEGRAM_CHAT_ID no está definido"

client = Client(API_KEY, API_SECRET)
telegram_bot = Bot(token=TELEGRAM_TOKEN)

# Configuración
MARGEN_BENEFICIO = 0.004
STOP_LOSS = 0.03
TIEMPO_MAX_HORAS = 2
...
def seleccionar_monedas(memoria, blacklist):
    ...
    for ticker in tickers:
        ...
        if ("USDT" in simbolo 
            and not any(x in simbolo for x in ["UP", "DOWN", "BUSD", "USDC", "TUSD"]) 
            and not (simbolo in blacklist and datetime.fromisoformat(blacklist[simbolo]) + timedelta(hours=6) > ahora)):
...
def comprar_moneda(simbolo, cantidad_usdt, blacklist):
    try:
        ...
        if cantidad < min_qty:
            telegram_bot.send_message(chat_id=TELEGRAM_CHAT_ID,
                text=f"⚠️ {simbolo} no se puede comprar: cantidad ({cantidad}) < mínimo ({min_qty})")
            blacklist[simbolo] = str(datetime.now())
            return None
        orden = client.order_market_buy(symbol=simbolo, quantity=cantidad)
        return orden
    except Exception as e:
        telegram_bot.send_message(chat_id=TELEGRAM_CHAT_ID,
            text=f"❌ Error comprando {simbolo}: {str(e)}")
        blacklist[simbolo] = str(datetime.now())
        return None
...
def vender_moneda(simbolo, cantidad):
    try:
        ...
        telegram_bot.send_message(chat_id=TELEGRAM_CHAT_ID,
            text=f"❌ Error al vender {simbolo}: {str(e)}")
        return None
...
async def ciclo():
    ...
    memoria_dict = memoria if isinstance(memoria, dict) else {}
    blacklist = cargar_json(LISTA_NEGRA)
    ...
    while True:
        ...
        monedas = seleccionar_monedas(memoria_dict, blacklist)
        for simbolo in monedas:
            orden = comprar_moneda(simbolo, tope, blacklist)
            if orden:
                precio = float(client.get_symbol_ticker(symbol=simbolo)['price'])
        ...
        guardar_json(MEMORIA_IA, memoria_dict)
        guardar_json(LISTA_NEGRA, blacklist)
